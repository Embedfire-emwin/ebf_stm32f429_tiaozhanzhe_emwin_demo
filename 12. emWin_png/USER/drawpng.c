...
*       _GetData
*/
static int _GetData(void * p, const U8 **ppData, unsigned int NumBytesReq, U32 Off) {
...
*/
static void _DrawPNG_file(const char * sFilename, uint16_t usPOSX, uint16_t usPOSY) 
{
/* 打开文件 */		
...
void drawpng(void)
{
  ...
  GUI_Clear(); 
	...
	GUI_PNG_Draw(_acPNG, sizeof(_acPNG),50,300);
	GUI_Delay(100);
	_DrawPNG_file("1:/PNG/pic1.png",50,20);
	_DrawPNG_file("1:/PNG/pic2.png",400,20);
	_DrawPNG_file("1:/PNG/pic3.png",200,150);
	_DrawPNG_file("1:/PNG/pic4.png",600,150);
	GUI_Delay(100);
	    
}

/* 实际的测试需要是图像宽度的4倍即可，切记(也就是保证每个像素如果是32位数据的情况) */
static char _acBuffer[800*4];
/*
*********************************************************************************************************
*
*       _GetData
...
*********************************************************************************************************
*/
static int _bmpGetData(void * p, const U8 ** ppData, unsigned NumBytesReq, U32 Off) 
{
	static int FileAddress = 0;
	UINT NumBytesRead;
	FIL *PicFile;
...
*********************************************************************************************************
*/
static void _ShowBMPEx(const char * sFilename, uint16_t usPOSX, uint16_t usPOSY) 
{	    	
    /* 打开文件 */		
	result = f_open(&file, sFilename, FA_OPEN_EXISTING | FA_READ | FA_OPEN_ALWAYS);
  ...
  printf("file open ok\n");
	...
	GUI_BMP_DrawEx(_bmpGetData, &file, usPOSX, usPOSY);
...
void drawbmp(void)
{
  GUI_SetBkColor(GUI_WHITE);
  GUI_Clear();
	_ShowBMPEx("1:/Picture/789.bmp",0,0);
  printf("show 789.bmp\n");
	GUI_Delay(100);
}
...
static void _ShowJPGEx(const char * sFilename, uint16_t usPOSX, uint16_t usPOSY)  
{	    	
	/* 打开文件 */		
	result = f_open(&file, sFilename, FA_OPEN_EXISTING | FA_READ | FA_OPEN_ALWAYS);
	if (result != FR_OK)
  ...
  GUI_JPEG_DrawEx(_bmpGetData, &file, usPOSX, usPOSY);
...
static void _ShowJPG(const char * sFilename) 
{
  ...
  UINT bw;
	...
	/* 申请一块内存空间 并且将其清零 */
    hMem = GUI_ALLOC_AllocZero(1024*128);
	/* 将申请到内存的句柄转换成指针类型 */
